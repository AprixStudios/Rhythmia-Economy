        ...
        getDB(message.author.id).then(sender => {
            ...
            getDB(user.id).then(receiver => {
                ...
                saveDB(sender).then(saveDB(receiver).then(() => {
                    ...
                    if (sender.rep.lastRep <= Date.now() && realAmount >= 1000) {
                        message.channel.send(`${user} you can now add +1 rep to ${message.author}\nIf you would like to do so, do +rep`);

                        const repFilter = repResponse => {
                            return repResponse.author.id === user.id && repResponse.content.toLowerCase().startsWith('+rep') || repResponse.author.id === message.author.id && ['e-givemoney', 'e-give', 'e-give-money'].some(cmd => repResponse.content.toLowerCase().indexOf(cmd.toLowerCase()) >= 0);
                        }
                        ...
                        message.channel.awaitMessages(repFilter, { max: 1 }).then(repCollected => {
                            ...
                            if (repMSG.author.id === message.author.id) return;
                                ...
                                sender.rep.amount += 1;
                                sender.rep.lastRep = Date.now()+43200000;
